...
CourotineScope - область действия корутины, Dispatcher определяет какой поток будет использоваться корутиной:
...
* Были получены знания о запуске\остановке фоновых операций (coroutine/asynctask/thread). Старое решение задачи continue watch создавало много потоков, новое решение через Java Threads решило эту проблему прерыванием потока через interrupt(), решение через Async Task android studio отмечало как устаревшее, однако им можно пользоваться, нужно было переопределить doInBackground() и onProgressUpdate(). Решение через корутины требовало меньшее количество кода в onResume() запускался цикл с задержкой 1 секунду, который обновлял счётчик.
