...
AsyncTask в функции doInBackground() будет каждую секунду вызывать функцию onProgressUpdate(), которая и будет обновлять счётчик времени. Было проверено, что не появляется более одного потока для подсчёта времени при повороте экрана.
...
* Были получены знания о запуске\остановке фоновых операций (coroutine/asynctask/thread). Решение с помощью корутин возможно лишь на котлине, а вот решение через java threads и async task возмжно и на Котлине и на Java. Старое решение задачи continue watch создавало много потоков, новое решение через Java Threads решило эту проблему прерыванием потока через interrupt(), решение через Async Task android studio отмечало как устаревшее, однако им можно пользоваться, нужно было переопределить doInBackground() и onProgressUpdate(). Решение через корутины требовало меньшее количество кода в onResume() запускался цикл с задержкой 1 секунду, который обновлял счётчик.
